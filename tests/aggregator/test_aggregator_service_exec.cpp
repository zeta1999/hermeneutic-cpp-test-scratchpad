...
TEST_CASE("aggregator_service streams aggregated books from cex executables") {
  ...
  CAPTURE(view.exchange_count);
  CAPTURE(view.bid_levels.size());
  CAPTURE(view.ask_levels.size());
  CAPTURE(view.best_bid.price.toString(4));
  CAPTURE(view.best_ask.price.toString(4));
  CAPTURE(view.best_bid.quantity.toString(4));
  CAPTURE(view.best_ask.quantity.toString(4));
  CHECK(message.exchange_count() == feeds.size());
  CHECK(view.exchange_count == feeds.size());
  CHECK(view.bid_levels.size() == 1);
  CHECK(view.ask_levels.size() == 2);
  CHECK(view.best_bid.price == dec("100.00"));
  CHECK(view.best_bid.quantity == dec("1"));
  CHECK(view.best_ask.price == dec("101.00"));
  CHECK(view.best_ask.quantity == dec("1"));
  CHECK(view.best_ask.price > view.best_bid.price);
  CHECK(view.bid_levels.front().price == view.best_bid.price);
  CHECK(view.ask_levels.front().price == dec("101.00"));
  CHECK(view.ask_levels.front().quantity == dec("1"));
